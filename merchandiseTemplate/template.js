...
window.addEventListener('load', e => {
    ...
    createApp({
        ...
        sceneURL: params.load || 'template.glb',
...
function runCode(app, puzzles) {
    ...
    // 从URL中提取模型名
    const modelName = window.location.pathname.split('/').pop().split('.')[0];
    let modelPath = `/applications/merchandiseTemplate/model/${modelName}.glb`; // 首选 GLB 格式

    const sphere = app.scene.getObjectByName("Sphere"); // 确保你的Sphere已经有一个名字

    if (!sphere) {
        console.error('Sphere object not found!');
        return;
    }
    ...
    fetch(modelPath)
        .then(response => {
            if (!response.ok) {
                // 如果 GLB 文件不存在，尝试加载 GLTF 文件
                throw new Error('GLB file not found, trying GLTF');
            }
            return response.blob(); // 如果 GLB 文件存在，直接返回数据
        })
        .catch(error => {
            console.log(error.message);
            modelPath = `/applications/merchandiseTemplate/model/${modelName}.gltf`; // 切换到 GLTF 格式
            return fetch(modelPath); // 尝试加载 GLTF 文件
        })
        .then(response => {
            if (!response.ok) {
                throw new Error('GLTF file not found either');
            }
            return response.blob(); // 如果 GLTF 文件存在，直接返回数据
        })
        .then(blob => {
            // 使用 GLTFLoader 加载模型
            loader.load(modelPath, function(gltf) {
                ...
                // 调整模型位置和动画等逻辑
                console.log('Model loaded and added to the scene:', modelPath);
            });
        })
        .catch(error => {
...
const ambientLight = new v3d.AmbientLight(0x404040, 2); // 软化整个场景的光
app.scene.add(ambientLight);
	
	model.traverse(function(object) {
    if (object.isMesh) {
        object.material.metalness = 0.1; // 降低金属感
        object.material.roughness = 0.5; // 增加一点粗糙度
        object.material.needsUpdate = true; // 更新材质
    }
});
app.renderer.shadowMap.enabled = true;
app.renderer.shadowMap.type = v3d.PCFSoftShadowMap; // 柔和阴影
app.renderer.antialias = true; // 抗锯齿
...
model.traverse(function(object) {
    if (object.isMesh) {
        ...
        // 调整模型的位置，使其底部接触Sphere的顶部
        model.position.y = Math.abs(modelBottom);

        console.log('Model loaded and positioned correctly:', modelPath);
        ...
        model.traverse(function(child) {
            if (child.isMesh) {
                ...
                child.addEventListener('mouseover', () => child.userData.isPaused = true);
                child.addEventListener('mouseout', () => child.userData.isPaused = false);
            }
        });
